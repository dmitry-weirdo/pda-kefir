...
test.testEntity.TestEntity = function() {
	...
	var ENTITY_NAME = 'test.TestEntity'; // needed only if attachments are present
	...
	var CHOOSE_FIELD_ID_FIELD_ID = 'testEntity-chooseFid';
	...
	var attachmentsFieldsLoadCount; // число целиком загруженных на сервер панелей с аттачами // for attachmentFields
	var attachmentsFieldAttachments; // attachments for attachmentFields field
	...
	function getItems() {
		...
		// attachments field panel (file upload)
		var attachmentsFieldPanel = ru.kg.gtn.form.getMultiUploadPanel({ readOnly: disabled, attachments: attachmentsFieldAttachments }, ATTACHMENTS_FIELD_PANEL_ID, ENTITY_NAME, ATTACHMENTS_FIELD_ENTITY_FIELD_NAME, ru.kg.gtn.getId(testEntity), ATTACHMENTS_FIELD_PANEL_WIDTH, ATTACHMENTS_FIELD_PANEL_HEIGHT);
	...
	function saveButtonHandler() {
		panel.getForm().submit({
			...
			params: { attachmentsFieldIds : Ext.getCmp(ATTACHMENTS_FIELD_PANEL_ID).getAttachmentsIds() },
	...
	function loadAttachments(config, callback) {
		attachmentsFieldsLoadCount = 0;
		var id = config.id || ru.kg.gtn.getId(config.testEntity);

		ru.kg.gtn.ajaxRequest(ru.kg.gtn.form.ATTACHMENTS_LIST_URL, { entityId: id, entityName: ENTITY_NAME, entityFieldName: ATTACHMENTS_FIELD_ENTITY_FIELD_NAME }, function(json) {
			attachmentsFieldAttachments = json.results; // возвращается список
