...
Ext.ux.UploadPanel = Ext.extend(Ext.Panel, {
	...
	,initComponent:function() {
		...
		if('body' !== this.buttonsAt) {
			if(Ext.isObject(this.customButton)) this[this.buttonsAt] = [addCfg, upCfg, this.customButton, '->', removeAllCfg];
            else this[this.buttonsAt] = [addCfg, upCfg, '->', removeAllCfg];
		}
		// }}}
		// {{{
		// create store
...
//					+ '<div class="ux-up-indicator">&#160;</div>'
	...
	,onRemoveFile:function(record) {
		...
		// remove DOM elements
		var inp = record.get('input');                                   beforefileremove
		var wrap = inp.up('em');
		inp.remove();
		if(wrap) {
			wrap.remove();
