...
function dhtmlXTreeObject(htmlObject, width, height, rootId) {
	...
	this.rowSelectHandler = null; // added by pda for handling row select
...
dhtmlXTreeObject.prototype._attachChildNode = function(parentObject, itemId, itemText, itemActionHandler, image1, image2, image3, optionStr, childs, beforeNode) {
	...
	if (this.XMLsource)if ((childs) && (childs != 0))Nodes[Count].XMLload = 0;
...
dhtmlXTreeObject.prototype._parseXMLTree = function(dhtmlObject, node, parentId, level) {
	...
	if (!level)
	{
		...
		if (dtmlXMLLoaderObject.loadHandler)
			dtmlXMLLoaderObject.loadHandler();
	}
...
dhtmlXTreeObject.prototype._getCountStatus = function(itemId, itemObject) {
	try
	{
		...
		if (itemObject.htmlNode.childNodes[0].childNodes[1].nodem.id == itemId)if (!itemObject.id)return 2;
		else return 1;
...
dhtmlXTreeObject.prototype._HideShow = function(itemObject, mode) {
	...
	if (Count > 1)
	{
		if (((Nodes[1].style.display != "none") || (mode == 1)) && (mode != 2))nodestyle = "none";
		else nodestyle = "";
...
dhtmlXTreeObject.prototype.onRowSelect = function(e, htmlObject, mode) {
	...
	// added by pda
	if (e && e.target && dtmlXMLLoaderObject.rowSelectHandler) // todo: replace this bydlocode with good config hanlder initialization
		dtmlXMLLoaderObject.rowSelectHandler(e)
...
dhtmlXTreeObject.prototype._createItem = function(acheck, itemObject, mode) {
	...
	itemObject.span.id = "label_" + itemObject.id;//	added by pda
...
dhtmlXTreeObject.prototype.setOnClickHandler = function(func) {
	if (typeof(func) == "function")this.aFunc = func;
	else this.aFunc = eval(func);
};
...
dhtmlXTreeObject.prototype.setOnCheckHandler = function(func) {
	if (typeof(func) == "function")this.checkFuncHandler = func;
	else this.checkFuncHandler = eval(func);
};


dhtmlXTreeObject.prototype.setOnOpenHandler = function(func) {
	if (typeof(func) == "function")this.openFuncHandler = func;
	else this.openFuncHandler = eval(func);
};


dhtmlXTreeObject.prototype.setOnDblClickHandler = function(func) {
	if (typeof(func) == "function")this.dblclickFuncHandler = func;
	else this.dblclickFuncHandler = eval(func);
};
...
dhtmlXTreeObject.prototype._checkParenNodes = function(itemId, htmlObject, shtmlObject) {
	...
	if (htmlObject.parentObject)return this._checkParenNodes(itemId, htmlObject.parentObject);
	else return 0;
};
...
dhtmlXTreeObject.prototype.setDragHandler = function(func) {
	if (typeof(func) == "function")this.dragFunc = func;
	else this.dragFunc = eval(func);
};
