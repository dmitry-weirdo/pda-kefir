...
public class ZipUtils
{
	...
	private static void zipDir(String rootDirPath, String dirPath, ZipOutputStream zos) throws IOException {
		...
		if (zipDir.list().length == 0)
		{
			...
			String zipDirName = concat(zipDir.getPath().substring(rootDirPath.length() + FileUtils.FILE_SEPARATOR.length()), FileUtils.STRAIGHT_FILE_SEPARATOR); // нужно добавить "/" к имени директории, чтобы она сохран€лась
			...
			ZipEntry anEntry = new ZipEntry(zipDirName); // отрезаем корневую директорию, иначе пишетс€ абсолютный путь файла (полна€ структура директорий)
		...
		for (File file : zipDir.listFiles())
		{
			...
			{
				// отрезаем корневую директорию, иначе пишетс€ абслютный путь файла (полна€ структура директорий)
	...
	/**
	 * –азархивирует zip-файл с заданным именем в заданную директорию.
	 * @param fileName полное им€ zip-файла.
	 * @param dirPath директори€, в которую извлекаетс€ zip-файл.
	 * @return список имен разархивированных файлов.
	 * @throws RuntimeException ошибка извлечени€.
	 */
			...
			while (entries.hasMoreElements())
			{
				...
				{ // extract file
					...
					if ( !FileUtils.fileExists(filePath) )
					{
						...
						// непуста€ директори€ может отсутствовать как отдельна€ сущность в zip-файле, поэтому создаем родителскую директорию извлекаемого файла
