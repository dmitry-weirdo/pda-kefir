...
test.testEntity.TestEntity = function() {
	...
	var CREATE_SAVE_ERROR_MSG = 'Ошибка при создании тестовой сущности';
	var CREATE_CANCEL_BUTTON_ID = 'testEntity-cancel';
	var CREATE_CANCEL_BUTTON_TEXT = 'Отмена';

//	var SHOW_URL = '/testEntityGet'; // todo: think whether this is needed
	...
	var CHOOSE_FIELD_CHOOSE_BUTTON_TEXT = 'Выбрать';
	var CHOOSE_FIELD_SHOW_BUTTON_ID = 'testEntity-chooseField-show';
	var CHOOSE_FIELD_SHOW_BUTTON_TEXT = 'Просмотреть';

	var SAVE_BUTTON_ID = 'testEntity-save';

	var FIELDS_IDS = [
		STR_FIELD_FIELD_ID, INT_FIELD_FIELD_ID, INT_SPINNER_FIELD_FIELD_ID,
		DOUBLE_FIELD_FIELD_ID, DATE_FIELD_FIELD_ID, BOOLEAN_FIELD_FIELD_ID,
		ENUM_FIELD_FIELD_ID, COMBO_FIELD_FIELD_ID,
		CHOOSE_FIELD_CHOOSE_BUTTON_ID, // for choose field use button
		SAVE_BUTTON_ID, SHOW_CANCEL_BUTTON_ID
	];

	var CHOOSE_FIELD_FIELDS = [ // поля, заполняемые при выборе связанной сущности
		[ 'chooseField', 'id' ],
	...
	var panel;
	var action;
	var testEntity; // подставляемый из таблицы
	var fillFormFunction;
	var successHandler;

	// заполнение из форм выбора
	function fillChooseFieldFields(chooseField) {
		var form = panel.getForm();
		ru.kg.gtn.form.fillFormFields(form, chooseField, CHOOSE_FIELD_FIELDS);
		Ext.getCmp(CHOOSE_FIELD_SHOW_BUTTON_ID).setDisabled(false); // сделать доступной кнопку "Просмотр"
	...
	function fillFormFields() {
		...
		// todo: если нужно, указывать renderer
...
//		ru.kg.gtn.form.fillFormField(form, testEntity, [ 'intSpinnerField', 'intField' ] ); // случай, когда названия полей сущности и формы различны
		ru.kg.gtn.form.fillFormField(form, testEntity, 'intSpinnerField');
		ru.kg.gtn.form.fillFormField(form, testEntity, 'doubleField');
		ru.kg.gtn.form.fillFormField(form, testEntity, 'dateField');
	...
	function getItemsLayout(config) { // возвращает разметку полей, получая конфиг с полями
		var chooseFieldFieldSet = {
			...
			items: [
				// default markup for choose field
				...
				// 2nd row
				, ru.kg.gtn.form.getFormPanel({ width: 450 }, 150, [ config.chooseFieldShortNameField ])
		...
		return [ // по дефолту возвращается просто набор полей как есть
			...
			, config.chooseFieldIdHiddenField // holds id for chooseField (Здесь, чтобы не портить табличную разметку в fieldset)
			, chooseFieldFieldSet

//			// default markup for choose field
//			config.chooseFieldIdHiddenField, // holds id for chooseField
//			ru.kg.gtn.form.getColumnPanel({}, [ // 1st string with choose button
//				ru.kg.gtn.form.getFormPanel({ width: 400 }, 150, [ config.chooseFieldNameField ]),
//				config.chooseFieldChooseButton
//				config.chooseFieldShowButton
//			]),
//			ru.kg.gtn.form.getColumnPanel({}, [
//				ru.kg.gtn.form.getFormPanel({ width: 400 }, 150, [ config.chooseFieldShortNameField ])
//			])
		];
	}
	function getItems() {
		...
		var idHiddenField = ru.kg.gtn.form.getHiddenField({}, ID_FIELD_ID, 'id'); // hidden field for id
		...
		var intSpinnerField = ru.kg.gtn.form.getSpinnerField({ disabled: disabled, maxLength: 2, minValue: 10, maxValue: 90 }, INT_SPINNER_FIELD_FIELD_ID, 'intSpinnerField', 'Целое спиннер поле', 15); // последнее - исходное значение
		...
		var dateField = ru.kg.gtn.form.getDateField({ disabled: disabled, minValue: '01.01.2010', maxValue: '02.02.2012' }, DATE_FIELD_FIELD_ID, 'dateField', 'Датовое поле', false);
		...
		var comboField = ru.kg.gtn.form.getComboBox(
			{
		...
		var chooseFieldShowButton = new Ext.Button({
			...
			style: { marginLeft: 10, marginBottom: 5 },
			handler: function() {
				su.opencode.minstroy.leasing.developer.Developer.initShowForm({
					id: panel.getForm().findField('chooseField').getValue() // get id from chosenField's hidden id value
				});
			}
		});
	...
	function getSaveErrorMsgTitle() {
		switch (action)
		{
			...
			default: ru.kg.gtn.showError('Incorrect action for getSaveErrorMessageTitle: ' + action); return null;
		}
	}
	function getSaveButtonText() {
		switch (action)
		{
			...
			default: ru.kg.gtn.showError('Incorrect action for getSaveButton: ' + action); return null;
		}
	}
	...
	function getButtons() {
		var cancelButton = getCancelButton(); // cancelButton is always present
	...
	function getDefaultButton() {
		switch (action)
		{
			...
			default: return undefined; // null defaultButton causes error of Ext.Window
		}
	}
