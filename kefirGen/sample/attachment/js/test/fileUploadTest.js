...
test.testEntity.FileUpload = function() {
	...
	var ENTITY_NAME = 'test.TestEntity';
	var testEntityId; // передается в форму в конфиге
	var attachments; // список приложений сущности, загружаемый для fileUploadPanel с сервера
	// eo
	...
	function getFileUploadPanel() {
		return ru.kg.gtn.form.getMultiUploadPanel({ attachments: attachments }, 'testEntity-attachments', ENTITY_NAME, testEntityId, 300);
	}
	...
	function getAttachments(callback) {
		// todo: // move attachments list to common constant near ru.kg.gtn.MultiUploadPanel
		ru.kg.gtn.ajaxRequest('/attachmentsList', { entityId: testEntityId, entityName: ENTITY_NAME }, function(json) {
			attachments = json.results; // возвращается список
	...
	function initWindow() {
		window = new Ext.Window({
			...
			items: [ /*gridPanel, */getFileUploadPanel() ]
		});
	}
	function showWindow() {
		initWindow();
	...
	return {
		init: function(config) {
			...
			// todo: testEntityId = config.entityId or config.testEntityId
			testEntityId = 36;
			getAttachments(function() { initGridPanel(showWindow); } );
		}
	}
}();