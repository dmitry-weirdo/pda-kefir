...
/**
 * ƒобавл€ет реализациюю методов сервиса в указанный класс, реализующий сервис.
 * <br/>
 * ≈сли класс не существует, он создаетс€ с помощью {@linkplain su.opencode.kefir.gen.fileWriter.LocalServiceBeanFileWriter LocalServiceBeanFileWriter}
 * <br/>
 * ≈сли класс реализации сервиса существует, то методы добавл€ютс€ в место уканное {@linkplain su.opencode.kefir.gen.fileWriter.LocalServiceBeanFileWriter#APPEND_METHOD_IMPLEMENTATION_MARKER маркером}.
 */
public abstract class ServiceBeanMethodAppender extends Appender
{
	...
	/**
	 * ƒобавл€ет необходимые импорты дл€ методов реализации сервиса.
	 * @param fileLines строки, считанные из файла, добавл€ть нужно в их конец, позици€ строк уже стоит перед маркером.
	 * @param imports строки, содержащие уже имеющиес€ импорты (нужно провер€ть, что они не совпадают с добавл€емыми импортами).
	 */
	protected abstract void appendImports(List<String> fileLines, List<String> imports);

	/**
	 * @return список сигнатур методов, добавл€емых в сервис.
	 */
	protected abstract List<String> getMethodSignatures();

	/**
	 * ƒобавл€ет реализации методов сервиса перед указанным маркером.
	 * @param fileLines строки, считанные из файла, добавл€ть нужно в их конец, позици€ строк уже стоит перед маркером.
	 */
	protected abstract void appendMethods(List<String> fileLines);

	/**
	 * ƒобавл€ет пол€ реализации сервиса.
	 * @param fileLines строки, считанные из файла, добавл€ть нужно в их конец, позици€ строк уже стоит перед маркером.
	 * @param fields строки, содержащие уже имеющиес€ private пол€ (нужно провер€ть, что они не совпадают с добавл€емыми полд€ми по имени).
	 */
	...
	protected void appendEjbField(String serviceClassFullName, List<String> fileLines, List<String> fields) {
		if ( serviceClassFullName.equals( getServiceClassName(extEntity, entityClass) ) ) // исключить добавление ссылки сервиса на самого себ€
			return;
