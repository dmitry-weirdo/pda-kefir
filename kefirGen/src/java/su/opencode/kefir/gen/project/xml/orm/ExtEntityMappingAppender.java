...
public class ExtEntityMappingAppender extends EntityMappingAppender
{
	...
	private void appendTable(List<String> fileLines) {
		...
		if (tableName.equals(entityClass.getName())) // таблицы с аналогичными классу именами отдельно объ€вл€ть не надо
			return;
	...
	private void appendAttributes(List<String> fileLines) {
		...
		for (Field field : entityClass.getDeclaredFields())
		{
			...
			{
				appendId(fileLines, field.getName(), getOrmSequenceName(extEntity, entityClass)); // todo: возможность в IdField тоже указывать им€ sql-столбца
				continue;
			}
			...
			if ( hasTextAreaFieldAnnotation(field) )
			{
				...
				{ // lob поле -> надо добавить еще указание <lob/>, вне зависимости от названи€ пол€
					appendLobBasic(fileLines, field.getName());
				}
				else
				{
					appendBasic(fileLines, field.getName()); // simple field -> add basic column name, if needed // todo: возможность перегружать дефолтное им€ sql-колонки во всех пол€х
				}
